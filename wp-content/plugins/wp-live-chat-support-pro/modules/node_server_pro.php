...
add_action("wplc_hook_admin_menu_layout", "wplc_node_basic_compat_version_check", 1);
/*
...
function wplc_node_basic_compat_version_check(){
    ...
    if ($checker < 8000) { 
        ...
        <?php
    } else if($checker < 8025){
        $wplc_error_string = sprintf( __( 'WP Live Chat Support Pro 8.0.11 <strong>requires WP Live Chat Support version 8.0.25</strong> (basic version). Please <strong><a href="%1$s">update the plugin</a></strong> in order for the plugin to continue working correctly.', 'wplivechat' ),
        './update-core.php'
