...
/**
 * 8.0.12 - 2019-04-10 - High priority
 * Fixes security exploit in file upload handler. Thanks to Active Intelligence
 *
 * 8.0.11 - 2019-03-19 - Medium priority
 * Modified server integration code
 * Modified remote file sources
 * Added update notice to ensure stability
 * 
 * 8.0.10 - 2019-03-04 - Low Priority
 * Added Advanced Tools - Export/Import Functionality
 * Added call to action animations for chat bubble
 * Added ability to select the default chat icon within settings area
 * Added compatibility for Advanced Access Manager plugin
 * Added support for WP Approve User
 * Fixed Chat Agent Online number in admin header not updating correctly
 * Fixed bug where user selected department did not reflect in the chat dashboard
 * Fixed Quick Response ordering in live chat admin
 * Fixed bug where duplicate new chat notifications were being sent via email
 * Modified all Gravatar links to default to Mystery Man
 * Tested logo size and placement module
...
require_once (plugin_dir_path(__FILE__) . "modules/cta_animations_pro.php");
...
function wplc_control_logged_out() {
    if(!isset($_GET['role'])){
    	add_action('pre_get_users', 'wplc_advanced_access_manager_compatibility', 1000);
		$check = get_users(array(
			'meta_key' => 'wplc_chat_agent_online',
		));
    	remove_action('pre_get_users', 'wplc_advanced_access_manager_compatibility', 1000);
 ...
 */
function wplc_advanced_access_manager_compatibility($query) {
        ...
        <tr class='wplc-icon-area'>
            ...
            <td>
				...
				<div id="wplc_default_chat_icons" style="display: none">
					...
					<img class="wplc_default_chat_icon_selector" src="<?php echo(plugin_dir_url(__FILE__) . 'images/chaticon.png') ?>">	
					<img class="wplc_default_chat_icon_selector" src="<?php echo(plugin_dir_url(__FILE__) . 'images/default_icon_1.png') ?>">	
					<img class="wplc_default_chat_icon_selector" src="<?php echo(plugin_dir_url(__FILE__) . 'images/default_icon_2.png') ?>">
					<img class="wplc_default_chat_icon_selector" src="<?php echo(plugin_dir_url(__FILE__) . 'images/default_icon_3.png') ?>">
				</div>
...
if (!function_exists("wplc_acbc_hook_control_push_js_to_front")) { 
	...
	function wplc_acbc_hook_control_push_js_to_front() {
    	...
    	if(!empty($wplc_pro_settings["wplc_pro_auto_first_response_chat_msg"])){
    		wp_localize_script('wplc-user-pro-features', 'wplc_pro_auto_resp_chat_msg', "" . apply_filters('wplc_pro_auto_first_response_chat_msg_filter', $wplc_pro_settings["wplc_pro_auto_first_response_chat_msg"])."");
    	}
...
add_action('wplc_hook_styling_setting_bottom', 'wplc_pro_additional_styling_settings', 99);
/**
* Renders additional settings 
*/
function wplc_pro_additional_styling_settings(){
 	$wplc_pro_settings = get_option("WPLC_PRO_SETTINGS");
 	$wplc_pro_auto_first_response_chat_msg = isset($wplc_pro_settings['wplc_pro_auto_first_response_chat_msg']) ? $wplc_pro_settings['wplc_pro_auto_first_response_chat_msg'] : '';
 	?>	
 	<tr>
...
add_filter('wplc_pro_setting_save_filter', 'wplc_pro_additional_styling_settings_save');
/**
 * Saves pro settings which are a part of the styling tab
*/
function wplc_pro_additional_styling_settings_save( $settings ) {
	if(isset($_POST['wplc_save_settings'])){
		if(isset($_POST['wplc_pro_auto_first_response_chat_msg'])){
			$settings['wplc_pro_auto_first_response_chat_msg'] = esc_attr($_POST['wplc_pro_auto_first_response_chat_msg']);
		} else {
			$settings['wplc_pro_auto_first_response_chat_msg'] = "";
		}
		return $settings;
	}
}